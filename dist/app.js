...
const hitsIndicator = document.querySelector('.game-indicator');
const infoGame = document.querySelector('.game-goal');
const backFaceCardImg = document.querySelectorAll('.card-back img');
const fruitsAndVegs = ['apple', 'banana', 'cherry', 'straw', 'brocoli', 'pepper'];
let boardGame = [[], [], []];
let nTimesFAndVUsed = {
    apple: 0,
    banana: 0,
...
function setImgBackFaceCard(nRow, nColumn, value) {
    const cardWrapper = cardsWrapper[nRow * 4 + nColumn];
    const backFaceImg = cardWrapper.querySelector('.card-back img');
